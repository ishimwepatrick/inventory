...
const SalesDashboard = ({systemData}) => {
	const {sales, salesLoading, shops, shopsLoading, userData, categories, weekly, mondy, lmondy, sundy, lsundy, findSeries, methodColors, statusColors, avatarColors, payments, paymentsLoading} = systemData;

	let credits = 0;	
	new Promise(function(myResolve, myReject) {
		credits = salesLoading ? 0 : sales.docs.filter(x=> x.data().amount > x.data().paid && x.data().source === userData.userStore).reduce((accumulator, currentElement) => {
	...
	let TodayCash = 0;
	new Promise(function(myResolve, myReject) {
		TodayCash = paymentsLoading ? 0 : payments.docs.filter(x=> x.data().date >= new Date().setHours(0,0,0,0) && x.data().source === userData.userStore  && x.data().type === 1).reduce((accumulator, currentElement) => {
			return parseFloat(accumulator) + parseFloat(currentElement.data().amount);
		},0)
	});
	...
	return (
		<>
			{userData.role !== 'Operator' ? <Row gutter={16}>
				<Col xs={24} sm={24} md={24} lg={16} xl={15} xxl={14}>
				<WeeklyRevenue categories={categories} weekly={weekly} weekData = {salesLoading ? [] : sales.docs.filter(x=> x.data().date >= mondy && x.data().date <= sundy && x.data().source === userData.userStore)} lastWeekData = {salesLoading ? [] : sales.docs.filter(x=> x.data().date >= lmondy && x.data().date <= lsundy && x.data().source === userData.userStore)} findSeries={findSeries}/>
				</Col>
				<Col xs={24} sm={24} md={24} lg={8} xl={9} xxl={10}>
					<DisplayDataSet credits={credits} customer={TodayCash} loading={salesLoading} todayData = {salesLoading ? [] : sales.docs.filter(x=> x.data().date >= new Date().setHours(0,0,0,0) && x.data().source === userData.userStore)} />
				</Col>
			</Row> : null}
			<Row gutter={16}>
				<Col xs={24} sm={24} md={24} lg={24}>
					<Card title="Recent Orders">
						<List 
							products={salesLoading ? [] : sales.docs.filter(x => x.data().source === userData.userStore).reverse().slice(0,9)} 
