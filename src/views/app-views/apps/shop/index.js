...
const Quantity = ({systemData}) => {
	...
	let {products, productsLoading, records, recordsLoading, userData, shortTypes, rounder } = systemData;
	records = recordsLoading ? [] : records.docs.filter(x => x.data().refunded === false && x.data().source === userData.userStore && x.data().date < +(new Date().getTime()));
	let productRecords = [];
	if(!productsLoading && !recordsLoading){
		let allRecords = [];
		products.docs.filter(x => x.data()[userData.userStore] === true).map((doc) => {
			...
			if(doc.data().usedStore !== undefined){
				doc.data().usedStore.split('**;;@!*.').map(store =>{
					...
					return initialStock += doc.data()[`${store}pcsStock`] + (doc.data()[`${store}pkgStock`]*doc.data().qtyPkgBk);
				})
			}
			...
			let sqMeter = (productType === '1' && pkgUnit ===  '0') ? ' and ' + parseInt(sqMeters % ctnSqm) + ' ' + shortTypes[productType] : ''; 
		...
		return (
			<Card>
				...
				<List products={productsLoading ? [] : products.docs.filter(x => x.data()[userData.userStore] === true)} loading={productsLoading} brandloading={brandloading} productRecords={productRecords}/>
			</Card>
		)
}
