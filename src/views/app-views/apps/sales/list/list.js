...
const List = (props) => {
		const { showInvoice, products, loading, loadingRecord, methodColors, statusColors, users, userLoading, avatarColors, shops} = props;
		const tableColumns = [
			{
				title: 'Date',
				dataIndex: 'date',
		...
		return (
				<Table columns={shops.length > 1 ? tableColumns : tableColumn} dataSource={products} loading={loading} rowKey='id' size="small"/>
		)
}
