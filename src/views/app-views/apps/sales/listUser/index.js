...
const Purchases = ({systemData}) => {
	...
	const onSearch = e => {
		...
		const data = _.filter(searchArray, (item) => item.data().customerName.toLowerCase().includes(value.toLowerCase()) || item.data().customerNumber.includes(value));
		setList(data)
		// setSelectedRowKeys([])
	}
	...
	return (
		...
		<Card className={(visible) ? 'd-none' : ''}>
			<Flex alignItems="center" justifyContent="between" mobileFlex={false}>
				<Flex className="mb-1" mobileFlex={false}>
					<div className="mr-md-3">
						<Title level={4}>{userData.role !== 'Operator' ? 'Orders received' : 'Pending Orders'}</Title>
					</div>
				</Flex>
				<Flex className="mb-1" mobileFlex={false}>
			...
			<List products={userData.role !== 'Operator' ? list.filter(x => x.data().source === userData.userStore).reverse() : list.filter(x => x.data().source === userData.userStore && !x.data().status).reverse()} loading={salesLoading} stores={shops} sloading={shopsLoading} showInvoice={showInvoice} loadingRecord={recordsLoading} methodColors={methodColors} statusColors={statusColors} users={users} userLoading={userLoading} avatarColors={avatarColors}/>
		</Card>
