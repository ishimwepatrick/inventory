...
const Sales = ({systemData}) => { 	
	...
	let {products, productsLoading, records, recordsLoading, singleShop, multiShop, stores, shops, shopsLoading, storesLoading, userData, shortTypes, getId } = systemData;
	...
	const handleSubmit = async () => {
		...
		try {
			...
			setSalesInfo(data);
			let shopDetails = stores.docs.filter(x=> x.id === shopCode);
			let ShopName = (shopDetails.length === 1) ? {'name':shopDetails[0].data().name,'location':shopDetails[0].data().location} : {'name':shopCode,'location':shopCode}
	  ...
	  const handleproductNameChange = (index, value) => {
		...
		if(!shop){
			...
			message.destroy();
			message.error({content:"Select Shop to continue", key});
			return;
		}
			...
			<Card>
				...
				>
					<div className="d-md-flex justify-content-md-between">
						<div className="">
							<h2 className="mb-1 font-weight-semibold">New Sale Form</h2>
							<Form.Item
								name="shopCode"
