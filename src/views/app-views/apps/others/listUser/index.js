...
const Refill = ({systemData}) => {
	...
	const {recordsLoading, shops, shopsLoading, expenses, expensesLoading, userData, avatarColors, methodColors } = systemData;
	...
	if(!expensesLoading){
	  let allCustomer = [];
	  expenses.docs.filter(x=> x.data().amount > x.data().paid && x.data().source === userData.userStore).map((doc) => {   
	  ...
	  const showSales = (customer) => {
		...
		setCustomerData(expenses.docs.filter(x=> x.data().amount > x.data().paid && x.data().supplierNumber === customer.number && x.data().source === userData.userStore))
	  ...
	  const handleOk = async () => {
		try {
			...
			expenses.docs.filter(x=> x.data().amount > x.data().paid && x.data().supplierNumber === customer && x.data().source === userData.userStore).map((doc) => {   
				let balance = parseFloat(doc.data().amount - doc.data().paid);
