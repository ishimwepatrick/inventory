...
const Refill = ({systemData}) => {
	let { move, moveLoading, records, recordsLoading, shops, shopsLoading, userData, shortTypes, users, userLoading, avatarColors } = systemData;
	...
	return (
		<>
			<Card className={(visible) ? 'd-none' : ''}>
				<Flex alignItems="center" justifyContent="between" mobileFlex={false}>
					<Flex className="mb-1" mobileFlex={false}>
						<div className="mr-md-3">
							<Title level={4}>Shops Refill</Title>
						</div>
					</Flex>
					<div>
				...
				<List products={moveLoading ? [] : move.docs.filter(x=> x.data().source === userData.userStore).reverse()} loading={moveLoading} stores={shops} sloading={shopsLoading} recordsLoading={recordsLoading} showInvoice={showInvoice} users={users} userLoading={userLoading} avatarColors={avatarColors}/>
			</Card>
