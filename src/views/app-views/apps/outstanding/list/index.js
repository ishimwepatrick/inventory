...
const Refill = ({systemData}) => {
	...
	const { records, recordsLoading, shops, shopsLoading, purchases, singleShop, multiShop, stores, storesLoading, purchasesLoading, avatarColors, getId, userData, methodColors } = systemData;
	...
	if(!purchasesLoading){
	  let allCustomer = [];
	  purchases.docs.filter(x=> x.data().amount > x.data().paid).map((doc) => {   
	...
	const showInvoice = (employee) => {
	  ...
	  const showSales = (customer) => {
		...
		setCustomerData(purchases.docs.filter(x=> x.data().amount > x.data().paid && x.data().supplierNumber === customer.number))
	  ...
	  const handleOk = async () => {
		try {
			...
			purchases.docs.filter(x=> x.data().amount > x.data().paid && x.data().supplierNumber === customer).map((doc) => {   
				let balance = parseFloat(doc.data().amount - doc.data().paid);
