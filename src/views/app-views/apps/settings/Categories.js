...
import { DataService } from "services/category.service";
...
const EditableTable = ({systemData}) => {
  ...
  const { category, categoryLoading } = systemData;
  ...
  const handleOk = async e => {
    try {
      ...
      DataService.create(categoryData)
      setVisible(false);
      setConfirmLoading(false);
      form.setFieldsValue({newCategory:null});
    } catch (errInfo) {form.setFieldsValue({newCategory:null});}
  ...
  const save = async key => {
    try {
      ...
      DataService.update(key,categoryData)
      setEditingKey('');
      ...
      render: (_, record) => {
        ...
        return editable ? (
          ...
          <Popconfirm title="Sure to delete?" onConfirm={e => {
              ...
              <a href="/#" className="ml-2"><DeleteOutlined /> Delete</a>
            </Popconfirm>
          </>
        );
      },
  ...
  return (
    <Form form={form} component={false}>
      ...
      <Table
        ...
        dataSource={!categoryLoading ? category.docs : []} 
