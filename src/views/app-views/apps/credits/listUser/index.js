...
const Refill = ({systemData}) => {
	...
	const { records, recordsLoading, shops, shopsLoading, sales, salesLoading, userData, avatarColors, getId, methodColors } = systemData;
	...
	if(!salesLoading){
	  let allCustomer = [];
	  sales.docs.filter(x=> x.data().amount > x.data().paid && x.data().source === userData.userStore).map((doc) => {   
	...
	const showInvoice = (employee) => {
	  ...
	  const showSales = (customer) => {
		...
		setCustomerData(sales.docs.filter(x=> x.data().amount > x.data().paid && x.data().customerNumber === customer.number && x.data().source === userData.userStore))
	  ...
	  const handleOk = async () => {
		try {
			...
			sales.docs.filter(x=> x.data().amount > x.data().paid && x.data().customerNumber === customer && x.data().source === userData.userStore).map((doc) => {   
				let balance = parseFloat(doc.data().amount - doc.data().paid);
	...
	return (
		...
		{!invoice &&(<Card>
			<Flex alignItems="center" justifyContent="between" mobileFlex={false}>
				...
				{type ? (<div>
					<Button className="mr-2" onClick={() => {setActiveCustomer(customerInfo)}} size="small" block>Add Payment</Button>
					<Button onClick={listBack} size="small" type="primary" icon={<ArrowLeftOutlined />} block>Back</Button>
				</div>) : ''}
			</Flex>
