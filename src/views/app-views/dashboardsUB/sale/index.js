...
const SalesDashboard = ({systemData}) => {
	const {sales, salesLoading, shops, shopsLoading, userData, categories, weekly, mondy, lmondy, sundy, lsundy, findSeries, methodColors, avatarColors} = systemData;
	...
	new Promise(function(myResolve, myReject) {
		...
		TotalCredits = salesLoading ? 0 : sales.docs.filter(x=> x.data().amount > x.data().paid && x.data().source === userData.userStore).reduce((accumulator, currentElement) => {
		...
		<>
			<Row gutter={16}>
				...
				<Col xs={24} sm={24} md={24} lg={8} xl={9} xxl={10}>
					<DisplayDataSet credits={credits} customer={TotalCredits} loading={salesLoading} todayData = {salesLoading ? [] : sales.docs.filter(x=> x.data().date >= new Date().setHours(0,0,0,0) && x.data().source === userData.userStore)} />
				</Col>
			</Row>
			<Row gutter={16}>
